...
if [ ! -f "apps/bot/.env" ]; then
    if [ -f "apps/bot/.env.example" ]; then
        ...
        sed -i.bak 's/^NODE_ENV=production$/NODE_ENV=development/' apps/bot/.env
        rm -f apps/bot/.env.bak
        echo "Created apps/bot/.env from template for local development."
    fi
fi
...
echo ""
echo "Next steps:"
echo "  1. Edit apps/bot/.env and apps/mini-app/.env.local with your credentials"
echo "  2. Provision PostgreSQL separately and set DATABASE_URL (Compose currently provides Redis/gateway only)"
echo "  3. Optional local Redis: docker compose up -d redis"
echo "  4. Run migrations: cd packages/db && pnpm db:migrate"
echo "  5. Start bot: cd apps/bot && pnpm dev"
echo "  6. Start mini-app: cd apps/mini-app && pnpm dev"
echo ""
