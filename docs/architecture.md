...
- `src/lib/telegram.ts` treats Telegram as an optional host adapter and passes signed launch data only to Privy's authentication flow. The official bridge loads before application scripts as Telegram specifies, while bridge absence never gates public routes; a Telegram launch never falls through to injected-wallet discovery, and a configured Privy boundary waits for a delayed bridge before mounting so signed auth cannot be missed. Ordinary browsers retain the explicit injected-wallet login path.
- Transaction reviews are portal-backed, route-stable sheets: the review body scrolls internally while the acknowledgement/confirm rail remains visible above app navigation. `components/review/` owns focus containment, progress announcements, execution-result copy, and Telegram-aware explorer links; route forms remain orchestration only.
