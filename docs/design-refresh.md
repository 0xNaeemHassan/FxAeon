...
The interface is a focused, wallet-owned workspace for f(x) Protocol. This pass is a conservative refinement of the existing routes: it changes presentation, responsive hierarchy, and recovery clarity without changing the transaction engine or supported capability surface.

The reference observations and measured snapshot in this document are historical evidence from 3 September 2026. Current launch, action, and release status are maintained in [`architecture.md`](architecture.md), [`testing.md`](testing.md), and [`implementation-progress.md`](implementation-progress.md).
...
- History skeletons with the same geometry as loaded history, followed by a concise empty state for the burner wallet.
...
- Product routes fit their primary content and action rail above the fixed navigation at supported phone and desktop sizes. `/docs` may scroll as a reading surface, and an expanded mobile Trade chart may scroll when its data needs the room; no other route requires page scrolling to finish an action.
...
1. All application routes, plus the requested read-only guide: Portfolio at `/` with `/portfolio` compatibility, login setup, trade, positions, earn, borrow, move, more, settings, history, receive, and docs. The independently deployed landing is in `apps/landing` for fxaeon.xyz; the financial app uses fxaeon.com. `/activity` remains only a compatibility redirect to `/history`. The normal web flow uses in-place wallet controls from Portfolio; `/login` is retained as a standalone setup surface. Adding documentation does not expand the 15-method transaction surface.
...
7. Fresh browser/fork evidence for the Edit → debounced read-only Live preview → final rebuild/simulation → Awaiting signature → Submitted → Confirming → Confirmed/Failed flow, signing boundaries, confirmation, and post-confirmation position discovery when the interface changes affect these flows.
...
## Historical verified local evidence — 3 September 2026
...
Shared position cards and Borrow summaries show estimated position value (collateral USD less debt USD), with both legs visible. Calculations retain bigint accounting units and the SDK's returned decimals until rounding to cents. These are display estimates, not P&L, liquidation values, or close quotes.
...
Price validation is independent per token. A delayed fxUSD quote no longer hides fresh ETH/BTC prices; missing quotes receive bounded single-contract CoinGecko fallback requests with adaptive rate-limit retry/backoff. The merged market header is the single price authority in the UI: it has no source badge or duplicate spot/chart quote. Chart provenance is documented, not repeated as a competing price label. Neither feed affects transaction planning, and no stablecoin peg is invented.
