...
1. All 12 application routes, plus the requested read-only guide: landing, login, portfolio, trade, positions, earn, borrow, move, more, settings, activity, receive, docs. Adding documentation does not expand the 15-method transaction surface.
2. Dark and light themes, including portaled token/wallet surfaces and saved first paint.
...
- The complete local `pnpm verify` gate passed: scope guard (15 SDK methods, 13 routes), lint, type checks, dependency audit, static export/CSP, bundle limits, 167 unit/security tests, and 40 browser tests. Coverage includes owned-quantity USD precision, disconnected token pickers at 320px, and keyboard/touch behavior for leverage controls. Three protected fork tests are intentionally skipped by the ordinary gate and were run separately below.
- `pnpm test:anvil:all` passed all three suites at Ethereum block `25893155`: 100 snapshot/revert iterations, 100 ordered-route stress iterations, and the real four-position protocol proof. The provider URL is excluded from the evidence.
- `pnpm test:anvil:browser` passed on the same block. Actual app controls prepared, reviewed, simulated, and confirmed ETH/BTC long and short positions; ownership, nonzero collateral/debt, delayed discovery, cross-workspace reads, available USDC before each trade, post-confirmation balance refresh, and position USD labels were verified before snapshot restoration. Only disposable fork funds were used. The final picker value layout and slider centering are presentation-only follow-ups covered by the final ordinary gate.
- The final visual capture set covers all 13 routes in both themes at 390×844 and 1440×1000 (52 captures). The follow-up USD/balance layout sweep uses visibly labelled illustrative market data for deterministic layout inspection only; it is not published financial evidence. Automated responsive checks additionally cover 320, 360, 375, 412, and 430px widths. Reference-browser/device behavior is not implied by headless coverage.
...
Price validation is independent per token. A delayed fxUSD quote no longer hides fresh ETH/BTC prices; missing quotes receive a bounded, cached CoinGecko fallback with rate-limit backoff. Neither feed affects transaction planning, and no stablecoin peg is invented. Trade charts retain a concise CoinGecko source link without the former market eyebrow or display-only suffix.
