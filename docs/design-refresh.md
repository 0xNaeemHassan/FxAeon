...
1. All application routes, plus the requested read-only guide: root/Portfolio, login setup, trade, positions, earn, borrow, move, more, settings, history, receive, and docs. There is no app landing page; `/activity` remains only a compatibility redirect to `/history`. The normal web flow uses in-place wallet controls from Portfolio; `/login` is retained as a standalone setup surface. Adding documentation does not expand the 15-method transaction surface.
...
7. Fresh browser/fork evidence for the in-card Edit → Live preview → Review → Awaiting signature → Submitted → Confirming → Confirmed/Failed flow, signing boundaries, confirmation, and post-confirmation position discovery when the interface changes affect these flows.
...
Price validation is independent per token. A delayed fxUSD quote no longer hides fresh ETH/BTC prices; missing quotes receive one bounded, batched CoinGecko fallback with adaptive rate-limit retry/backoff. The merged market header is the single price authority in the UI: it has no source badge or duplicate spot/chart quote. Chart provenance is documented, not repeated as a competing price label. Neither feed affects transaction planning, and no stablecoin peg is invented.
