...
type HarnessState = {
  ...
  deferNextPlan: boolean; executionResolvers: Array<() => void>;
  prepareCount: number; planCount: number; runnerCount: number; sendCount: number;
...
const H = (globalThis as typeof globalThis & { __actionReviewHarness?: HarnessState }).__actionReviewHarness ??= {
  ...
  version: 1, mode: initialMode ?? 'auto', failNextPrepare: false, executeVersion: undefined, deferNextPlan: false, executionResolvers: [],
  prepareCount: 0, planCount: 0, runnerCount: 0, sendCount: 0,
...
function Harness() {
  ...
  const planBuilder = useCallback(async () => {
    ...
    if (H.deferNextPlan) {
      H.deferNextPlan = false;
      return new Promise<typeof route>((resolve) => H.executionResolvers.push(() => resolve(route)));
    }
    return route;
  }, [version]);
  ...
  return <>
    ...
    <div role="toolbar">
      ...
      <button type="button" onClick={() => { H.deferNextPlan = true; }}>Defer next execution</button>
      <button type="button" onClick={() => { H.executeVersion = version + 1; }}>Change on execute</button>
    ...
    />
    <output data-metrics>{JSON.stringify({ prepare: H.prepareCount, plan: H.planCount, runner: H.runnerCount, send: H.sendCount, refreshStarted: H.refreshStarted, completeStarted: H.completeStarted })}</output>
  </>;
}
