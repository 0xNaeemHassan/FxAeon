...
test.describe("Telegram-sized accessibility contract", () => {
  test("every official screen fits a narrow viewport and exposes a main landmark", async ({ page, requests }) => {
    for (const route of ROUTES) {
      ...
      await expect(page.locator("main"), `${route} main landmark`).toBeVisible();
      const geometry = await page.evaluate(() => ({
        viewport: document.documentElement.clientWidth,
        content: document.documentElement.scrollWidth,
        lang: document.documentElement.lang,
      }));
      expect(geometry.content, `${route} must not overflow horizontally`).toBeLessThanOrEqual(geometry.viewport + 1);
  ...
  test("visible app controls meet the mobile target size", async ({ page, requests }) => {
    for (const route of ROUTES) {
      ...
      expect(undersized, `${route} visible controls smaller than 44px`).toEqual([]);
    }
    assertNoBackendRequests(requests);
  });
